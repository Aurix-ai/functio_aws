...
def start_server(clickhouse_server_path: str = "/home/ubuntu/ClickHouse_debug/build_debug/programs/clickhouse"):
    ...
    _server_process = subprocess.Popen(
        ...
        cwd="/home/ubuntu/ClickHouse_debug/build_debug/programs"  # Important for table visibility
    )
...
def stop_server():
    ...
    if _server_process is not None:
        ...
        try:
            _server_process.wait(timeout=10)  # Add timeout to avoid hanging
        except subprocess.TimeoutExpired:
            _server_process.kill()  # Force kill if terminate doesn't work
            try:
    ...
    if result.returncode == 0 and result.stdout.strip():
        ...
        for pid in pids:
            try:
                subprocess.run(["sudo", "kill", "-9", pid], check=True, timeout=10)
            except subprocess.CalledProcessError as e:
                print(f"Warning: Failed to kill existing clickhouse process {pid}: {e}", file=sys.stderr)
            except subprocess.TimeoutExpired:
                print(f"Warning: Timeout killing process {pid}", file=sys.stderr)
