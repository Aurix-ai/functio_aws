...
def parse_short_function_name(full_name: str) -> str:
    ...
    # If the string contains an unterminated template (a '<' with no matching '>'),
    # our template-aware splitter can get "stuck" inside the template and fail to
    # split on later '::'. In that case, fall back to a simple delimiter split.
    level = 0
    for ch in full_name:
        if ch == '<':
            level += 1
...
def function_lookup(function_name, file_path, executable):
    ...
    print("short function name", short_function_name)
    ...
    if result:
        ...
        print(f"Function '{function_name}' () not found using traditional way. Trying nothing more. Giving up")
        return None
