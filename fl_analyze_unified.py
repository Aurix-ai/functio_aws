...
Usage:
    python fl_analyze_unified.py --executable "command args" [options]
    python fl_analyze_unified.py --pid 1234 [options]

Command Line Flags:
    --executable COMMAND    The executable command to profile (quote if it has arguments)
    --pid PID              Process ID to profile (mutually exclusive with --executable)
    --duration SECONDS     Duration to record in seconds (default: 10, only used with --pid)
    --api-key API_KEY      OpenAI API key (or set OPENAI_API_KEY environment variable)
    --output-base NAME     Base name for output files (default: flamegraph)
    --full-paths           Show full absolute paths instead of project-relative paths
    --exclude-dir PATH     Exclude functions from specified directory (can be used multiple times)

Requirements:
    - flame_folded.sh must be executable and in the current directory
    - OpenAI API key (set via --api-key or OPENAI_API_KEY environment variable)
...
def run_flamegraph(executable_command=None, pid=None, duration=10, output_base="flamegraph"):
    """Run flame_folded.sh to generate flame graph folded file.
    ...
    """
    flame_script = Path("./flame_folded_new.sh")
    
    if not flame_script.is_file():
        raise FileNotFoundError("flame_folded.sh not found in current directory")
    ...
    try:
        ...
        if result.stderr and any(marker in result.stderr for marker in ['Exception', 'Error:', 'DB::Exception', 'error']):
        ...
        if result.returncode != 0:
            ...
            print(f"stdout: {result.stdout}", file=sys.stderr)
            print(f"stderr: {result.stderr}", file=sys.stderr)
            raise subprocess.CalledProcessError(result.returncode, cmd_args, output=result.stdout, stderr=result.stderr)
        ...
        print(f"Error: FlameGraph generation timed out after 15 seconds", file=sys.stderr)
        if stdout_output:
...
def extract_source_locations(perf_data_path, full_paths=False):
    ...
    try:
        ...
        cmd = [
            'perf', 'report', '--stdio', '-i', perf_data_path, 
...
def analyze_flamegraph_strategy_5(folded_content, api_key=None, source_locations=None, output_base="flamegraph", full_paths=False, excluded_dirs=None, verbose=False, prefix_path=None, custom_prefix=None):
    ...
    for func, samples, percentage, func_type in result_functions:
        ...
        # Transform path: strip prefix and add custom prefix if provided
        location = transform_path(location, prefix_path, custom_prefix)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate flame graph and analyze using Wide Plateau Identification strategy"
    )
    
    # Create mutually exclusive group for executable vs PID
    target_group = parser.add_mutually_exclusive_group(required=True)
    target_group.add_argument(
        "--executable", 
        help="The executable command to profile (quote if it has arguments)"
    )
    target_group.add_argument(
        "--pid", 
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Duration to record in seconds (default: 10, only used with --pid)"
    )
    parser.add_argument(
        "--api-key", 
    ...
    parser.add_argument(
        "--output-base", 
        default="flamegraph",
        help="Base name for output files (default: flamegraph)"
    )
    parser.add_argument(
        "--full-paths",
        action="store_true",
        help="Show full absolute paths in function locations instead of converting to project-relative paths"
    )
    parser.add_argument(
        "--exclude-dir",
        action="append",
        help="Exclude functions from the specified directory path (can be used multiple times)"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Show detailed analysis information (default: show only function list)"
    )
    ...
    parser.add_argument(
        "legacy_executable", 
        nargs="?",
        help="(Legacy) The executable command to profile - use --executable instead"
    )
    
    args = parser.parse_args()
    
    # Handle legacy positional argument
    executable_command = args.executable or args.legacy_executable
    
    # Get API key
    api_key = args.api_key or os.getenv("OPENAI_API_KEY")
    if not api_key:
        print("Error: OpenAI API key required. Use --api-key or set OPENAI_API_KEY", file=sys.stderr)
        sys.exit(1)
    
    try:
        # Generate flame graph folded file
        if args.pid:
            folded_path = run_flamegraph(pid=args.pid, duration=args.duration, output_base=args.output_base)
        else:
            folded_path = run_flamegraph(executable_command=executable_command, output_base=args.output_base)
        
        # Extract source locations from perf.data
        perf_data_path = "perf.data"
        if args.verbose:
            print("[*] Extracting source locations from perf.data...")
        source_locations = extract_source_locations(perf_data_path, full_paths=args.full_paths)

        # Apply enhanced demangling to the folded file
        if args.verbose:
            print("[*] Applying enhanced C++ demangling...")
        enhanced_folded_path = apply_enhanced_demangling(folded_path, args.verbose)
        
        # Read folded file content
        if args.verbose:
            print("[*] Reading folded file for OpenAI analysis...")
        folded_content = read_folded_file(enhanced_folded_path)
        ...
        if args.verbose:
            ...
            print("="*80)
        analysis = analyze_flamegraph_strategy_5(folded_content, api_key, source_locations, args.output_base, full_paths=args.full_paths, excluded_dirs=args.exclude_dir, verbose=args.verbose)
...
def unified_strategy(query , output_base, verbose, full_paths, exclude_dir, prefix_path=None, custom_prefix=None):
    ...
    try:
        ...
        if verbose:
            ...
            print("WIDE PLATEAU IDENTIFICATION")
            print("="*80)
        analysis = analyze_flamegraph_strategy_5(folded_content, api_key, source_locations, output_base, full_paths=full_paths, excluded_dirs=exclude_dir, verbose=verbose, prefix_path=prefix_path, custom_prefix=custom_prefix)
